...
def parse_bandit(output):
    ...
    for line in output.split('\n'):
        if line.startswith('>> Issue: '):
            msg = line.split('>> Issue: ')[1].split(' \n')[0] if ' \n' in line else line.split('>> Issue: ')[1]
        elif line.startswith('   Location: '):
            parts = line.split('   Location: ')[1].split(':')
            if len(parts) >= 2:
                file_path = parts[0]
                issues.append({'file': file_path, 'tool': 'bandit', 'msg': msg, 'severity': 'critical'})
    return issues
...
def inject_tasks(issues):
    ...
    for issue in issues:
        ...
        task_str = f"- **[ ] TASK:** auto-audit-{task_hash} | [DEBT] | **Loc:** {file} | **Spec:** Fix {tool} error: {msg} | **Deps:** None | **LOC Estimate:** 10\n"
