...
- **db-001-1-upload-handler**: Implement chunked file upload handler with hash validation (Location: src/modules/db/ingestion.py)
- **db-001-2-metadata-extractor**: Extract raw metadata and store in temporary staging table (Location: src/modules/db/ingestion.py)
- **db-001-verify-ingestion**: Unit tests for upload handling and metadata extraction (Location: tests/test_db_ingestion.py)
- **db-002-1-validation-view**: Implement base QWidget for validation interface (Location: src/gui/pyqt/tabs/validation_tab.py)
- **db-002-2-gamification-logic**: Implement point calculation and achievement logic (Location: src/services/gamification.py)
- **db-002-verify-validation**: Unit tests for gamification logic and UI state transitions (Location: tests/test_db_validation.py)
- **db-003-1-musicbrainz-sync**: Implement async MusicBrainz seed crawler (Location: src/services/metadata/aggregators.py)
- **db-003-2-spotify-sync**: Implement async Spotify metadata fetcher (Location: src/services/metadata/aggregators.py)
- **db-003-verify-aggregators**: Mock-based tests for external API sync logic (Location: tests/test_db_aggregators.py)
- **db-004-1-ssl-preprocessing**: Implement audio-to-tensor normalization for SSL (Location: src/modules/neu/training/ssl_pipeline.py)
- **db-004-2-ssl-dataloader**: Implement custom PyTorch Dataset for audio tensors (Location: src/modules/neu/training/ssl_pipeline.py)
- **db-004-3-ssl-training-loop**: Implement base self-supervised training loop with checkpointing (Location: src/modules/neu/training/ssl_pipeline.py)
- **db-004-verify-ssl**: Unit tests for preprocessing and training loop state (Location: tests/test_db_ssl.py)
- **db-audit-ingestion**: Analyze ingestion success rates and data quality (Location: scripts/audit_ingestion.py)
- **db-cleanup-stage-1**: Implement cron for orphaned staging record deletion (Location: scripts/cleanup.sh)
- **agent-001-1-router-logic**: Implement confidence threshold comparison logic (Location: src/modules/agent/orchestrator.py)
- **agent-001-2-task-dispatcher**: Build dispatcher for routing tasks to local vs cloud queues (Location: src/modules/agent/orchestrator.py)
- **agent-002-1-llm-client**: Implement generic HTTP client for OpenAI/Anthropic (Location: src/services/ai/llm_orchestrator.py)
- **agent-002-2-prompt-factory**: Implement dynamic template-based prompt generation (Location: src/services/ai/llm_orchestrator.py)
- **api-001-1-fastapi-init**: Bootstrap FastAPI app with middleware (Location: src/modules/api/main.py)
- **api-001-2-endpoints-metadata**: Implement GET/POST endpoints for metadata CRUD (Location: src/modules/api/main.py)
- **api-001-3-endpoints-file-sync**: Implement streaming endpoints for audio file sync (Location: src/modules/api/main.py)
- **agent-analyze-latency**: Implement middleware to track Edge-Cloud roundtrip latency (Location: src/utils/perf/latency_logger.py)
- **agent-verify-gateway**: Integration test for agent-router-gateway path (Location: tests/test_agent_gateway.py)
- **api-cleanup-routes**: Prune expired JWT tokens and invalid session routes (Location: src/modules/api/cleanup.py)
- **audio-002-1-chroma-features**: Extract CQT-based chroma features (Location: src/services/audio/dsp_engine.py)
- **audio-002-2-rhythm-extraction**: Implement BPM and onset strength detection (Location: src/services/audio/dsp_engine.py)
- **audio-003-1-mel-spectrogram**: Generate mel-spectrogram tensors from demixed stems (Location: src/utils/audio/spectrogram.py)
- **audio-cleanup-cache**: Implement cache eviction for demixed audio chunks (Location: src/utils/audio/cache.py)
- **audio-verify-dsp**: Unit tests for Chroma and Rhythm extraction accuracy (Location: tests/test_audio_dsp.py)
- **audio-audit-distortion**: Detect harmonic distortion in demixed stems (Location: scripts/audit_audio.py)
- **neu-001-1-instrument-resnet**: Define ResNet architecture for spectrogram classification (Location: src/services/ai/instrument_classifier.py)
- **neu-001-2-instrument-inference**: Implement inference wrapper and label mapping (Location: src/services/ai/instrument_classifier.py)
- **neu-002-1-vocal-stem-interface**: Implement interface to extract vocal stems from demixer stems (Location: src/services/ai/vocalist_analyzer.py)
- **neu-002-2-vocal-dvector**: Implement d-vector embedding generation via SpeechBrain (Location: src/services/ai/vocalist_analyzer.py)
- **neu-003-1-clap-raga-prompts**: Design and test 50+ Raga-specific zero-shot prompts (Location: src/services/ai/raga_classifier.py)
- **neu-analyze-model-drift**: Implement basic KL-divergence tracker for prediction distributions (Location: src/utils/ai/drift_detector.py)
- **neu-verify-inference**: Performance benchmark for all neural classifiers (Location: tests/test_neu_inference.py)
- **neu-cleanup-models**: Prune unused model checkpoints from cloud storage (Location: scripts/cleanup_models.sh)
- **data-001-1-schema-migration**: Implement idempotent SQL migration for v2 tables (Location: resources/db/schema_expansion_v2.sql)
- **data-audit-integrity**: Implement referential integrity check for the music graph (Location: scripts/audit_db.py)
- **data-cleanup-orphans**: Cleanup track metadata with missing local/cloud files (Location: src/services/cache/cleanup.py)
- **plg-001-dep-resolver**: Implement DependencyResolver via Kahn's Algorithm (Location: src/modules/plg/dependency_resolver.py)
- **plg-002-sandbox**: Implement PluginSandbox using MetaPathFinder (Location: src/modules/plg/plugin_sandbox.py)
- **plg-verify-isolation**: Verify plugin sandbox prevents unauthorized OS access (Location: tests/test_plg_isolation.py)
- **id-cleanup-history**: Prune play history older than 1 year (Location: src/modules/id/cleanup.py)
- **cld-004-cloud-ui**: Add cloud settings tab to configure provider (Location: src/modules/cld/ui.py)
- **cld-verify-connectivity**: Implement ping/validation for cloud providers (Location: src/modules/cld/test_connection.py)
- **resolve-003-skills-sync**: Implement integration hooks for run.sh skills (Location: scripts/run.sh)
- **resolve-004-audit-expansions**: Audit DB/Auth/API task expansions (Location: docs/planning/backlog.md)
- **resolve-005-dependency-audit**: Audit requirements.txt for pinned versions and vulnerabilities (Location: scripts/audit_deps.sh)
- **resolve-006-complexity-refactor**: Audit src/ for Cyclomatic Complexity > 10 (Location: scripts/check_complexity.sh)