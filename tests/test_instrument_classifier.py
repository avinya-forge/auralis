...
def test_resnet_init_and_forward():
    ...
    with patch("src.services.ai.instrument_classifier.nn", mock_nn), patch(
        "src.services.ai.instrument_classifier.torch", mock_torch
    ):
...
def test_inference_wrapper_classify():
    ...
    with patch("os.path.exists", return_value=True):
        wrapper.engine.run_classification = MagicMock(
            return_value=[{"label": "guitar", "score": 0.9}]
        )
        ...
        # Test mapping labels
        wrapper.engine.run_classification = MagicMock(
            return_value=[{"label": "unknown_inst", "score": 0.9}]
        )
...
def test_classifier_init_and_load():
    ...
    with patch("src.services.ai.instrument_classifier.torch", mock_torch), patch(
        "src.services.ai.instrument_classifier.nn", MagicMock()
    ):
...
def test_classifier_fine_tune():
    ...
    with patch("src.services.ai.instrument_classifier.torch", mock_torch), patch(
        ...
        with patch("src.services.ai.instrument_classifier.InstrumentResNet"):
...
def test_classifier_predict():
    ...
    with patch("src.services.ai.instrument_classifier.torch", mock_torch), patch(
        "src.services.ai.instrument_classifier.T", mock_T
        ...
        with patch("src.services.ai.instrument_classifier.InstrumentResNet"):
            ...
            mock_probs.squeeze.return_value.cpu.return_value.numpy.return_value = [
                0.1,
                0.8,
                0.1,
            ] + [0.0] * 7
