...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QPushButton,
                            ...
                            QRadioButton, QSpinBox, QLineEdit, QTextEdit,
...
from src.utils.file_utils import remove_empty_directories
from src.utils.config import get_config, set_config, save_config, create_env_example
...
class WorkerThread(QThread):
    ...
    def __init__(self, source_dirs, dest_dir, options, system_monitor, limit_files=None, dry_run=False, start_stage=1, end_stage=3):
        super().__init__()
        self.source_dirs = source_dirs
        self.dest_dir = dest_dir
        self.options = options
        self.system_monitor = system_monitor
        self.limit_files = limit_files  # Limit processing to specified number of files
        ...
        self.processed_files = []  # List of files that would be processed in dry run
        self.file_errors = {}  # Dict to track errors by file path
        self.scanned_files = []  # List of file info dictionaries from scan stage
        ...
        try:
            ...
            for i in range(total_files):
                if self.start_stage <= self.STAGE_SCAN:
                    ...
                    self.status_updated.emit(f"Scanning file {i+1} of {total_files}")
            ...
            results = {
                ...
                "stages_completed": [self.start_stage, min(self.end_stage, 3)]
            }
...
class MainWindow(QMainWindow):
    ...
    def setup_stage1_ui(self, parent_layout):
        ...
        self.extensions_edit = QLineEdit(get_config("FILE_EXTENSIONS", "mp3,flac,m4a,wav,aac,ogg,wma"))
    ...
    def setup_stage2_ui(self, parent_layout):
        ...
        self.audio_similarity_check.setToolTip("Analyzes audio content to find duplicate tracks regardless of filename or metadata")
        ...
        self.keep_duplicates_check.setToolTip("If unchecked, only the highest quality version of each duplicate will be kept")
        # Connect audio similarity checkbox to enable/disable keep duplicates option
    ...
    def setup_stage3_ui(self, parent_layout):
        ...
        self.lyrics_check.setToolTip("Fetch lyrics from online sources and embed them in the audio files")
        lyrics_layout.addWidget(self.lyrics_check)
        
        lyrics_info = QLabel("Lyrics will be embedded in the audio files so they can be displayed in music players like Apple Music.")
    ...
    def validate_destination(self):
        ...
        if self.dest_label.text() == "No destination selected":
            QMessageBox.warning(self, "Missing Destination", "Please select a destination directory.")
            return False
        return True
