...
class LyricsService:
                        ...
                        if song_path:
                            ...
                            if lyrics_response.status_code == 200:
                                ...
                                if lyrics_match:
                                    ...
                                    # Newer Genius format
                                    lyrics_json = re.search(r'__PRELOADED_STATE__ = JSON.parse\((.+?)\);</script>', html)
                                    if lyrics_json:
                                        json_str = lyrics_json.group(1).strip("'")
                                        try:
                                            json_data = json.loads(json_str)
                                            song_data = json_data.get('songPage', {}).get('lyricsData', {})
                                            lyrics = song_data.get('body', {}).get('html', '')
                                        except:
                                            return None
                                    else:
                                        # Try another approach
                                        lyrics_container = re.search(r'<div[^>]*class="[^"]*Lyrics__Container[^"]*"[^>]*>(.+?)</div>\s*</div>', html, re.DOTALL)
                                        if lyrics_container:
    ...
    def _fetch_from_musixmatch(self, artist: str, title: str) -> Optional[str]:
        ...
        try:
            ...
            # Extract lyrics using regex
            html = lyrics_response.text
            lyrics_match = re.search(r'<span class="lyrics__content__ok">(.+?)</span>', html, re.DOTALL)
            
            if not lyrics_match:
                # Try another pattern
                lyrics_match = re.search(r'<div class="mxm-lyrics"><span class="lyrics__content__[^"]*">(.+?)</span></div>', html, re.DOTALL)
