...
class MetadataSource:
    ...
    def update_stats(self, success, response_time):
        ...
        self.avg_response_time = self.total_response_time / self.total_count if self.total_count > 0 else 0.0
...
class MetadataService(QObject):
    ...
    def _load_stats(self):
        ...
        try:
            ...
            if stats_file.exists():
                ...
                if sum(source.total_count for source in self.sources.values()) >= self.learning_threshold:
    ...
    def _has_sufficient_metadata(self, file_info):
        ...
        for field in essential_fields:
            if field not in metadata or not metadata[field] or metadata[field].lower() == f"unknown {field}":
                return False
